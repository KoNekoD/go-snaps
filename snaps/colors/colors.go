...
func Fprint(w io.Writer, color, s string) {
	...
	_, _ = fmt.Fprintf(w, "%s%s%s", color, s, reset)
}
...
func FprintEqual(w io.Writer, s string) {
	...
	_, _ = fmt.Fprintf(w, "  %s%s%s", Dim, s, reset)
}
...
func FprintDelete(w io.Writer, s string) {
	...
	if hasNewlineSuffix(s) {
		...
		_, _ = fmt.Fprintf(w, "%s%s- %s%s", Reddiff, RedBg, s, reset)
	}
}
...
func FprintInsert(w io.Writer, s string) {
	...
	if hasNewlineSuffix(s) {
		...
		_, _ = fmt.Fprintf(w, "%s%s+ %s%s", Greendiff, GreenBG, s, reset)
	}
}
...
func FprintDeleteBold(w io.Writer, s string) {
	...
	_, _ = fmt.Fprintf(w, "%s%s%s%s", BoldRedBg, White, s, reset)
}
...
func FprintInsertBold(w io.Writer, s string) {
	...
	_, _ = fmt.Fprintf(w, "%s%s%s%s", BoldGreenBG, White, s, reset)
}
...
func FprintBg(w io.Writer, bgColor, color, s string) {
	...
	if hasNewlineSuffix(s) {
		...
		_, _ = fmt.Fprintf(w, "%s%s%s%s", bgColor, color, s, reset)
	}
}
